    ...
    <div className='home'>
      <div className='home__content' ref={homeContent}>
        <div className='content__start'>
          ...
          <button onClick={() => homeContent.current.scrollBy({ top: document.documentElement.clientHeight * 2, behavior: 'smooth' })}>Continue</button>
        </div>
        <div className='content__preview'>
          <div>INSERT TASKS PREVIEW</div>
          <button onClick={() => homeContent.current.scrollBy({ top: document.documentElement.clientHeight * 3, behavior: 'smooth' })}>Continue</button>
        </div>
      ...
      {isOffline && (
        <div>You are using the app offline.</div>
      )}
    </div>
  )
}
